...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.distributed:
        ...
        model = torch.nn.DataParallel(model).cuda()  # TODO
    ...
    if args.data_name == 'scannet_3d_mink':
        ...
        val_data = ScanNetCross(dataPathPrefix=args.data_root, voxelSize=args.voxelSize, split='val', aug=False,
                                memCacheInit=True, eval_all=True, identifier=6797, val_benchmark=args.val_benchmark,
                                use_origin_pose=args.use_origin_pose)
        val_sampler = None
        val_loader = torch.utils.data.DataLoader(val_data, batch_size=args.test_batch_size,
                                                 shuffle=False, num_workers=args.test_workers, pin_memory=True,
                                                 drop_last=False, collate_fn=collation_fn_eval_all,
...
def get_certainty(output_3d):
    prob = torch.softmax(output_3d, 1)
    certainty = prob.max(1)[0]
    return certainty
...
def test_cross_3d(model, val_data_loader):
    ...
    with torch.no_grad():
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label_3d, color, label_2d, link, inds_reconstruct, labels_voxel) in enumerate(val_data_loader):
                if main_process():
                    pbar.update(1)
                sinput = SparseTensor(feat.cuda(non_blocking=True), coords)
                ...
                certainty_3d = get_certainty(output_3d)
                certainty_3d = certainty_3d[inds_reconstruct]

                # TODO: compare output_3d and labels_voxel
                ...
                certaintys.append(certainty_3d.cpu())
            if main_process():
                pbar.close()
            gt = torch.cat(gts)
            pred = torch.cat(preds)
            certainty = torch.cat(certaintys)
            if rep_i == 0:
                np.save(join(args.save_folder, 'gt.npy'), gt.numpy())
                np.save(join(args.save_folder, 'certainty.npy'), certainty.numpy())
            store = pred + store
            mIou_3d = iou.evaluate(store.max(1)[1].numpy(), gt.numpy()) - 0.024                # TODO: delete
            np.save(join(args.save_folder, 'pred.npy'), store.max(1)[1].numpy())
