...
class LinkCreator(object):
    def __init__(self, fx, fy, mx, my, image_dim=(640, 480), voxelSize=0.05):  # TODO: image_dim
...
class ScanNetCross(ScanNet3D):
    VIEW_NUM = 3
    IMG_DIM = (640, 480)  # TODO: change image_dim
    ...
    def __init__(self, dataPathPrefix='Data', voxelSize=0.05,
        ...
        self.use_origin_pose = use_origin_pose
        ...
        for x in self.data_paths:
            if split == 'train':
                scene_path = x[:-15].replace('train', '2D')
                ps = glob(join(scene_path, 'render_color', '*.jpg'))
                assert len(ps) >= self.VIEW_NUM, \
                    '[!] %s has only %d frames, less than expected %d samples' % (x, len(ps), self.VIEW_NUM)
                ps.sort(key=lambda x: int(x.split('/')[-1].split('.')[0]))
                self.data2D_paths.append(ps)
            elif split == 'val':
                if self.use_origin_pose:
                    scene_path = x[:-15].replace('val', '2D')
                    ps = glob(join(scene_path, 'render_color', '*.jpg'))
                    assert len(ps) >= self.VIEW_NUM, \
                        '[!] %s has only %d frames, less than expected %d samples' % (x, len(ps), self.VIEW_NUM)
                    ps.sort(key=lambda x: int(x.split('/')[-1].split('.')[0]))
                    if val_benchmark:
                        ps = ps[::5]  # TODO: set skip frames
                    self.data2D_paths.append(ps)
                else:
                    scene_path = x[:-15].replace('val', '2D')
                    ps = glob(join(scene_path, 'render_color_mine', '*.jpg'))  # TODO: how to get rendered image
                    assert len(ps) >= self.VIEW_NUM, \
                        '[!] %s has only %d frames, less than expected %d samples' % (x, len(ps), self.VIEW_NUM)
                    ps.sort(key=lambda x: int(x.split('/')[-1].split('.')[0]))
                    self.data2D_paths.append(ps)

            # ps = glob(join(x[:-15].replace(split, 'render'), 'color', '*.jpg'))  # TODO: set 2D data path
            # assert len(ps) >= self.VIEW_NUM, '[!] %s has only %d frames, less than expected %d samples' % (
            #     x, len(ps), self.VIEW_NUM)
            # ps.sort(key=lambda x: int(x.split('/')[-1].split('.')[0]))
            # # TODO: change skip num
            # # if val_benchmark:
            # #     ps = ps[::5]
            # self.data2D_paths.append(ps)
        ...
        # TODO: set camera intrinsics
        self.linkCreator = LinkCreator(fx=577.87, fy=580.26, mx=319.5, my=239.5, image_dim=self.IMG_DIM,
                                       voxelSize=voxelSize)
        # 2D AUG
        value_scale = 255
    ...
    def __getitem__(self, index_long):
        ...
        labels_voxel = torch.from_numpy(labels_voxel).long()

        if self.eval_all:
            return coords, feats, labels, colors, labels_2d, links, inds_reconstruct, labels_voxel
    ...
    def get_2d(self, room_id, coords: np.ndarray):
        ...
        for v in range(self.VIEW_NUM):
            ...
            depth = imageio.imread(f.replace('render_color', 'render_depth').replace('jpg', 'png')) / 1000.0  # convert to meter
            ...
            if self.split == 'val' and self.use_origin_pose == False:
                posePath = f.replace('render_color_mine', 'viewpoint_mine').replace('.jpg', '.json')
                with open(posePath) as f1:
                    params = json.load(f1)
                extrin = np.asarray(params['extrinsic']).reshape((4, 4)).T
                pose = np.linalg.inv(extrin)
                # TODO: add labels
            else:
                posePath = f.replace('render_color', 'pose').replace('.jpg', '.txt')
                pose = np.asarray(
                    [[float(x[0]), float(x[1]), float(x[2]), float(x[3])] for x in
                     (x.split(" ") for x in open(posePath).read().splitlines())]
                )
                label = imageio.imread(f.replace('render_color', 'label-filt').replace('jpg', 'png'))
...
def collation_fn_eval_all(batch):
    ...
    :return:    coords: N x 4 (x,y,z,batch)
                ...
                links:  N x 4 x V (B,H,W,mask)
                inds_recons:ON
                labels_voxel: voxel_number

    """
    coords, feats, labels, colors, labels_2d, links, inds_recons, labels_voxel = list(zip(*batch))
    ...
    return torch.cat(coords), torch.cat(feats), torch.cat(labels), \
           torch.stack(colors), torch.stack(labels_2d), torch.cat(links), torch.cat(inds_recons), torch.cat(
        labels_voxel)
