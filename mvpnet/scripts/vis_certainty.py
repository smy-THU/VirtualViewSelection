...
for s in tqdm(scenes):
    ...
    certainty_this = certainty[l: l+v.shape[0]]
    ...
    color = (certainty_this-0.13)/0.87 * 255.0
    ori.elements[0].data['red'] = 255 - color
    ori.elements[0].data['green'] = color * 0.3
    ori.elements[0].data['blue'] = color * 0.7
    # ori.elements[0].data['green'] = -1
    ori.write(join(vis_prefix, name+'certainty.ply'))
    np.savetxt(join(vis_prefix, name+'certainty.txt'), certainty_this, fmt='%f')
    # pdb.set_trace()
